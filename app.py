...
# ⛔ SQLite file TIDAK BOLEH di Vercel
if os.getenv("VERCEL"):
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///:memory:'
else:
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///healthcare_portal.db'

app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
app.config['MAX_CONTENT_LENGTH'] = 16 * 1024 * 1024  # 16MB

# ⛔ Upload file TIDAK BOLEH di Vercel
if os.getenv("VERCEL"):
...
@app.before_request
def auto_login_demo():
    if os.getenv("VERCEL") and not current_user.is_authenticated:
        login_user(DemoUser())
...
def login():
    if os.getenv("VERCEL"):
        flash('Mode demo aktif: login otomatis')
...
def register():
    if os.getenv("VERCEL"):
        flash('Registrasi dinonaktifkan pada demo online')
        return redirect(url_for('dashboard'))
...
def upload_document():
    if app.config['UPLOAD_FOLDER'] is None:
        flash('Upload dinonaktifkan pada demo online')
        return redirect(url_for('dashboard'))
